...
Reproducible applied statistics _requires_ robust software:
...
- These are generally kept under the `tests` subdirectory.
- The [`testthat` package]() is a popular framework for unit testing.
...
```{r comment='#'}
library(testthat)
context("Testing a core function")
set.seed(175298)
test_that("What does this test do", {
  # contents of test, use the expect_*() family of functions
})
```
